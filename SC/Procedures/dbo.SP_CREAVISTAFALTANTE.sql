...
CREATE PROCEDURE [dbo].[SP_CREAVISTAFALTANTE] (@FE_CODIGO VARCHAR(50))   as


		if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[VVISTAFALTANTE'+@FE_CODIGO+']') and OBJECTPROPERTY(id, N'IsView') = 1)
	exec('drop view [dbo].[VVISTAFALTANTE'+@FE_CODIGO+']')
	
	exec ('CREATE VIEW dbo.VVISTAFALTANTE'+@FE_CODIGO+' 
	AS
