...
CREATE PROCEDURE [dbo].[SP_IMPORTACTUALIZAEQFACTIMP]   as
...
SET NOCOUNT ON 
	...
	/*==================================================================*/
		...
		WHERE     dbo.FACTIMP.FI_FOLIO IN (SELECT REPLACE(LEFT(RI_REGISTRO,CHARINDEX(',',RI_REGISTRO)-1),'FI_FOLIO = ','') 
		...
		WHERE dbo.FACTIMPDET.FID_PES_UNI>0 AND dbo.FACTIMPDET.ME_GEN in (select ME_KILOGRAMOS from configuracion) 
			AND dbo.FACTIMP.FI_FOLIO IN (SELECT REPLACE(LEFT(RI_REGISTRO,CHARINDEX(',',RI_REGISTRO)-1),'FI_FOLIO = ','') 
					FROM REGISTROSIMPORTADOS
					WHERE RI_REGISTRO LIKE 'FI_FOLIO%' AND RI_TIPO='I')
		...
		FROM  dbo.FACTIMPDET INNER JOIN
                            dbo.FACTIMP ON dbo.FACTIMPDET.FI_CODIGO = dbo.FACTIMP.FI_CODIGO
		WHERE dbo.FACTIMPDET.ME_GEN =  dbo.FACTIMPDET.ME_CODIGO AND
		  dbo.FACTIMP.FI_FOLIO IN (SELECT REPLACE(LEFT(RI_REGISTRO,CHARINDEX(',',RI_REGISTRO)-1),'FI_FOLIO = ','') 
			  FROM REGISTROSIMPORTADOS
			  WHERE RI_REGISTRO LIKE 'FI_FOLIO%' AND RI_TIPO='I')
