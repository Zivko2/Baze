...
-- actualiza el estatus de descarga despues de que ya esta hecha, es sobre la table de kardesped
CREATE PROCEDURE [dbo].[SP_ESTATUSKARDESPEDwo] (@kap_codigo int, @tipo char(1)='D')   as
...
@KAP_PADRESUST int, @KAP_CANTDESC decimal(38,6)
	...
	from KARDESPED where kap_codigo= @kap_codigo
...
/* total que se lleva descargado por hijo, unque use o no use equivalentes */	
	...
	begin
		...
		FROM dbo.KARDESPED WHERE (KAP_INDICED_FACT = @KAP_INDICED_FACT) 
...
/* total adescargar por hijo */
	...
	not exists (select * from kardesped where KAP_INDICED_FACT =@KAP_INDICED_FACT 
...
--						print 'descarga pendientes'
			...
			begin
				...
				(select * from KARDESPED where kap_padresust=@MA_HIJO and KAP_INDICED_FACT =@KAP_INDICED_FACT)
				begin
				...
				begin
					--componentes 'descarga individual - equivalentes'
					if exists (select * from KARDESPED where kap_saldo_fed=0 and KAP_INDICED_FACT =@KAP_INDICED_FACT 
				...
				if exists (SELECT * FROM KARDESPED WHERE (KAP_CODIGO IN
					...
					if exists (select * from KARDESPED where (KAP_INDICED_FACT = @KAP_INDICED_FACT) AND (KAP_PADRESUST = @kap_padresust)
						and kap_cantdesc >0)
					begin
						UPDATE KARDESPED
						SET KAP_ESTATUS='P'
			...
			if exists (select * from KARDESPED where kap_saldo_fed=0 and KAP_INDICED_FACT =@KAP_INDICED_FACT 
				...
				begin
					...
					if exists (select * from KARDESPED where kap_saldo_fed>0 and KAP_INDICED_FACT =@KAP_INDICED_FACT 
					      and (KAP_PADRESUST=@KAP_PADRESUST))
					and 
					exists (select * from KARDESPED where KAP_CANTDESC>0 and KAP_INDICED_FACT =@KAP_INDICED_FACT 
					      and  (KAP_PADRESUST=@KAP_PADRESUST))
...
--					print 'los componentes se descargan por equivalentes o x grupo'
					...
					begin
						...
						begin
							if exists (select * from KARDESPED where KAP_CANTDESC>0 and KAP_INDICED_FACT =@KAP_INDICED_FACT 
							      and kap_fiscomp='S' and (kap_padremain=@kap_padremain and kap_padresust=@kap_padresust))
							begin
								UPDATE KARDESPED
								SET KAP_ESTATUS='P'
				...
				if not exists (SELECT * FROM KARDESPED WHERE (KAP_CODIGO IN
				                          (SELECT     MAX(KAP_CODIGO) AS kap_codigo
				                            FROM          KARDESPED
				...
				and not exists (SELECT * FROM KARDESPED WHERE (KAP_CODIGO IN
				                          (SELECT     MAX(KAP_CODIGO) AS kap_codigo
				                            FROM          KARDESPED
...
--				print 'descarga individual - equivalentes - subensambles'
					if not exists (select * from KARDESPED where kap_saldo_fed>0 and KAP_INDICED_FACT =@KAP_INDICED_FACT 
