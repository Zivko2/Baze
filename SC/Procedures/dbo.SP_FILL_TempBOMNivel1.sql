...
CREATE PROCEDURE [dbo].[SP_FILL_TempBOMNivel1] (@BST_PT INT, @BST_HIJO INT, @BST_ENTRAVIGOR DATETIME, @nivel int, @BST_INCORPOR decimal(38,6), @mensaje char(1) output)   as

SET NOCOUNT ON 

declare @BST_HIJO1 int, @nivelr INT, @BST_INCORPOR1 decimal(38,6), @BST_INCORPOR2 decimal(38,6)
  ...
  BEGIN  
	...
	SET @BST_INCORPOR2 =@BST_INCORPOR*@BST_INCORPOR1
		...
		begin
			...
			insert into TempBOM_NIVEL(BST_PT, BST_PERINI, BST_NIVEL, BST_PERTENECE, BST_HIJO, BST_INCORPOR, BST_INCORPORUSO)
			values
			(@BST_PT, @bst_entravigor, @nivelr, @BST_HIJO, @BST_HIJO1, @BST_INCORPOR2, @BST_INCORPOR1)

		end
			if @@NESTLEVEL <32
			exec  SP_FILL_TempBOMNivel1 @BST_PT, @BST_HIJO1, @BST_ENTRAVIGOR, @nivelr, @BST_INCORPOR1, @mensaje=@mensaje output
			else
			begin
				set @mensaje='S'
				break
