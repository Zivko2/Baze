...
CREATE PROCEDURE [dbo].[SP_BOMAGREGA] (@pt int, @BST_hijo INT, @PerCambios CHAR(1), @Incorpor decimal(38,6), @Desp decimal(38,6), @Merma decimal(38,6))   as
...
IF @PerCambios ='S'
	BEGIN
		...
		if not exists (select * from bom_struct where bsu_subensamble=@pt and bst_hijo = @bst_hijo
				and bst_perini=@bm_entravigor and bst_perfin =@bm_perfin)
		BEGIN
