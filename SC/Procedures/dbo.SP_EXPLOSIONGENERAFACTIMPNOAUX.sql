...
GO
CREATE PROCEDURE [dbo].[SP_EXPLOSIONGENERAFACTIMPNOAUX] @tipofactura char(1), @FE_CODIGO int     as
...
@di_matriz int, @di_trafico int, @di_empresa int, @fe_folio varchar(30), @MA_GENERICO INT, @ME_GEN INT, @fi_tipocambio decimal(38,6), @OT_FOLIO varchar(25)
	...
	begin
		DECLARE CUR_IMPEXPLOSION CURSOR FOR
			...
			AND (TI_CODIGO<>1 AND TI_CODIGO<>2)
	end
	...
	BEGIN
		...
		INSERT INTO FACTIMPDET (FID_INDICED,FI_CODIGO,FID_NOPARTE,FID_COS_UNI, FID_COS_TOT, FID_CANT_ST,FID_PES_UNI, FID_PES_NET, FID_PES_BRU, FID_NOMBRE,FID_NAME,MA_CODIGO,
