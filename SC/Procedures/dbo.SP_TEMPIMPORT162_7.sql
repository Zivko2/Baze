...
GO
CREATE PROCEDURE [dbo].[SP_TEMPIMPORT162_7] AS
DECLARE @FACTEXP#FE_TIPO char,@FACTEXP#TQ_CODIGO smallint,@FACTEXP#TF_CODIGO smallint,@FACTEXPDET#FED_NOPARTEAUX varchar(8000),@FACTEXPDET#FED_COS_TOT float,@FACTEXP#FE_FOLIO varchar(8000),@FACTEXP#FE_FECHA datetime,@FACTEXPDET#FED_NOPARTE varchar(8000),@FACTEXPDET#FED_CANT float,@Cod_3 varchar(8000)
...
DECLARE CUR_TEMPIMPORT162_7 CURSOR FOR
SELECT MAX(Cod_3),FACTEXP7#FE_TIPO,MAX(FACTEXP7#TQ_CODIGO),MAX(FACTEXP7#TF_CODIGO),FACTEXP7#FE_FOLIO,MAX(FACTEXP7#FE_FECHA) FROM TEMPIMPORT162_7 WHERE FACTEXP7#FE_FOLIO NOT IN (select factexp.fe_folio from factexp WHERE factexp.fe_estatus in ('A', 'C', 'P', 'L', 'S') or fe_iniciocruce='S') GROUP BY FACTEXP7#FE_TIPO,FACTEXP7#FE_FOLIO
OPEN CUR_TEMPIMPORT162_7
FETCH NEXT FROM CUR_TEMPIMPORT162_7 INTO @Cod_3 ,@FACTEXP#FE_TIPO ,@FACTEXP#TQ_CODIGO ,@FACTEXP#TF_CODIGO ,@FACTEXP#FE_FOLIO ,@FACTEXP#FE_FECHA 
...
BEGIN
  ...
  BEGIN
          ...
          INSERT INTO FACTEXP (FE_CODIGO,FE_TIPO ,TQ_CODIGO ,TF_CODIGO ,FE_FOLIO ,FE_FECHA ,FE_DESCRIPTION1 ,FE_HEADER ,FE_FOOTER ,AG_MX ,AG_US ,CL_PROD ,DI_PROD ,CL_COMP ,DI_COMP ,CL_COMPFIN ,DI_COMPFIN ,CL_EXP ,DI_EXP ,CL_EXPFIN ,DI_EXPFIN ,CL_DESTINI ,DI_DESTINI ,CL_DESTFIN ,DI_DESTFIN ,CL_VEND ,DI_VEND ,CL_IMP ,DI_IMP ,PU_CARGA ,PU_SALIDA ,PU_ENTRADA ,PU_DESTINO ,CT_COMPANY1 ,CT_COMPANY2 ,IT_COMPANY1 ,MT_COMPANY1 ,IT_COMPANY2 ,MT_COMPANY2 ,FE_TIPOCAMBIO ,MO_CODIGO ,FE_PINICIAL ,FE_PFINAL ,AGT_CODIGO ) VALUES (@CONSECUTIVO,@FACTEXP#FE_TIPO ,@FACTEXP#TQ_CODIGO ,@FACTEXP#TF_CODIGO ,@FACTEXP#FE_FOLIO ,@FACTEXP#FE_FECHA ,@FACTEXP#FE_DESCRIPTION1,@FACTEXP#FE_HEADER,@FACTEXP#FE_FOOTER,@FACTEXP#AG_MX,@FACTEXP#AG_US,@FACTEXP#CL_PROD,@FACTEXP#DI_PROD,@FACTEXP#CL_COMP,@FACTEXP#DI_COMP,@FACTEXP#CL_COMPFIN,@FACTEXP#DI_COMPFIN,@FACTEXP#CL_EXP,@FACTEXP#DI_EXP,@FACTEXP#CL_EXPFIN,@FACTEXP#DI_EXPFIN,@FACTEXP#CL_DESTINI,@FACTEXP#DI_DESTINI,@FACTEXP#CL_DESTFIN,@FACTEXP#DI_DESTFIN,@FACTEXP#CL_VEND,@FACTEXP#DI_VEND,@FACTEXP#CL_IMP,@FACTEXP#DI_IMP,@FACTEXP#PU_CARGA,@FACTEXP#PU_SALIDA,@FACTEXP#PU_ENTRADA,@FACTEXP#PU_DESTINO,@FACTEXP#CT_COMPANY1,@FACTEXP#CT_COMPANY2,@FACTEXP#IT_COMPANY1,@FACTEXP#MT_COMPANY1,@FACTEXP#IT_COMPANY2,@FACTEXP#MT_COMPANY2,@FACTEXP#FE_TIPOCAMBIO,@FACTEXP#MO_CODIGO,@FACTEXP#FE_PINICIAL,@FACTEXP#FE_PFINAL,@FACTEXP#AGT_CODIGO)
               INSERT INTO REGISTROSIMPORTADOS (RI_REGISTRO,RI_TIPO,RI_CBFORMA, RI_USERID) VALUES                                               ('FE_FOLIO = '+CONVERT(varchar(100),@FACTEXP#FE_FOLIO)+', '+'FE_FECHA = '+CONVERT(varchar(100),@FACTEXP#FE_FECHA),'I',20,7) 
 if exists (select * from sysobjects where name='sysusrlog62')                 insert into sysusrlog62 (user_id, mov_id, referencia, frmtag, fechahora) values                                                        (7, 1 ,'FE_FOLIO = '+CONVERT(varchar(100),@FACTEXP#FE_FOLIO)+', '+'FE_FECHA = '+CONVERT(varchar(100),@FACTEXP#FE_FECHA),62, getdate()) 
        DECLARE CUR_TEMPIMPORT162_7DET CURSOR FOR
        SELECT MAX(Cod_3),FACTEXPDET7#FED_NOPARTEAUX,SUM(FACTEXPDET7#FED_COS_TOT),FACTEXPDET7#FED_NOPARTE,SUM(FACTEXPDET7#FED_CANT) FROM TEMPIMPORT162_7  WHERE FACTEXP7#FE_FOLIO=@FACTEXP#FE_FOLIO AND FACTEXP7#FE_TIPO=@FACTEXP#FE_TIPO AND FACTEXP7#TQ_CODIGO=@FACTEXP#TQ_CODIGO GROUP BY FACTEXPDET7#FED_NOPARTEAUX,FACTEXPDET7#FED_NOPARTE
        OPEN CUR_TEMPIMPORT162_7DET
                      FETCH NEXT FROM CUR_TEMPIMPORT162_7DET INTO @Cod_3 ,@FACTEXPDET#FED_NOPARTEAUX ,@FACTEXPDET#FED_COS_TOT ,@FACTEXPDET#FED_NOPARTE ,@FACTEXPDET#FED_CANT 
        WHILE (@@FETCH_STATUS = 0)
        BEGIN
