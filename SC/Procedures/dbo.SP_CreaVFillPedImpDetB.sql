...
CREATE PROCEDURE [dbo].[SP_CreaVFillPedImpDetB] (@pi_codigo int)   as

declare @picodigo varchar(50)

exec sp_droptable 'VFillPedImpDetB', 'V'

select @picodigo=convert(varchar(50),@pi_codigo)

exec('CREATE VIEW dbo.VFillPedImpDetB
with encryption as
...
WHERE     (dbo.PEDIMPDET.PID_IMPRIMIR = ''S'') AND dbo.PEDIMPDET.PI_CODIGO=0'+@picodigo+' 
