...
CREATE PROCEDURE [dbo].[SP_ACTUALIZAPID_PAGACONTRIB] (@pid_indiced int)   as

SET NOCOUNT ON 
declare @pa_origen  int, @ti_codigo int, @pi_codigo int, @CCP_TIPO varchar(5), @CL_VIRTPAGACONTRIB char(1), @PI_FEC_ENT datetime, @pagacontrib char(1),
...
select @ccp_tipo=ccp_tipo from configuraclaveped where cp_codigo=@cp_codigo
	...
	update pedimpdet
	set pid_pagacontrib=@pagacontrib
