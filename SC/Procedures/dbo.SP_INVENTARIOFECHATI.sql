...
CREATE PROCEDURE [dbo].[SP_INVENTARIOFECHATI] (@macodigo int, @cft_tipo char(1), @picodigo int, @fecha datetime)   as
...
-- se suma el saldo de los pedimentos 
	...
	WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') and dbo.PEDIMP.PI_MOVIMIENTO='E' 
		AND (dbo.CONFIGURATIPO.CFT_TIPO IN ('C', 'H', 'Q', 'X')) AND dbo.PEDIMP.PI_FEC_ENT<=@fecha
	GROUP BY  CASE WHEN dbo.PEDIMP.PI_TIPO IN ('C', 'A') THEN isnull(dbo.AGENCIAPATENTE.AGT_PATENTE, '') 
                      + '-' + dbo.PEDIMP.PI_FOLIO ELSE dbo.PEDIMP.PI_FOLIO END, dbo.PEDIMP.PI_FEC_PAG, MEDIDA_2.ME_CORTO, MEDIDA_1.ME_CORTO, 
	                      dbo.ARANCEL.AR_FRACCION, dbo.PEDIMPDET.PID_POR_DEF, dbo.PEDIMPDET.PID_DEF_TIP, dbo.SECTOR.SE_CLAVE, dbo.SPI.SPI_CLAVE, 
	                      dbo.PEDIMPDET.MA_CODIGO, dbo.PEDIMPDET.TI_CODIGO, dbo.PEDIMPDET.PID_NOPARTE, dbo.PEDIMP.PI_FEC_ENT, 
	...
	WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') AND (dbo.CONFIGURATIPO.CFT_TIPO IN ('C', 'H', 'Q', 'X')) AND dbo.VINVENTARIOKARSUM.FE_FECHA>=@fecha
		and dbo.PEDIMP.PI_MOVIMIENTO='E' and dbo.PEDIMP.PI_FEC_ENT <= @fecha
	GROUP BY  CASE WHEN dbo.PEDIMP.PI_TIPO IN ('C', 'A') THEN isnull(dbo.AGENCIAPATENTE.AGT_PATENTE, '') 
                      + '-' + dbo.PEDIMP.PI_FOLIO ELSE dbo.PEDIMP.PI_FOLIO END, MEDIDA_2.ME_CORTO, MEDIDA_1.ME_CORTO, dbo.ARANCEL.AR_FRACCION, 
	                      dbo.PEDIMPDET.PID_POR_DEF, dbo.PEDIMPDET.PID_DEF_TIP, dbo.SECTOR.SE_CLAVE, dbo.SPI.SPI_CLAVE, dbo.PEDIMPDET.MA_CODIGO, 
	                      dbo.PEDIMPDET.TI_CODIGO, dbo.PEDIMPDET.PID_NOPARTE, dbo.PEDIMP.PI_FEC_ENT, dbo.PEDIMP.PI_FEC_PAG, 
	                      dbo.PEDIMP.PI_CODIGO
	HAVING      (SUM(dbo.VINVENTARIOKARSUM.KAP_CANTDESC) > 0)end
else
begin
		insert into INVENTARIOTempFecha (NoPedimento,  FechaPedimento, FechaPagoPedimento,  PID_saldoCANT, ME_CORTO, 
	PID_SALDOGEN, ME_GEN, PID_COS_UNI, AR_FRACCION, PID_POR_DEF, MA_CODIGO, SE_CLAVE, SPI_CLAVE, TI_CODIGO, 
	...
	FROM         dbo.SPI RIGHT OUTER JOIN
	                      ...
	                      dbo.MEDIDA MEDIDA_1 ON dbo.PEDIMPDET.ME_CODIGO = MEDIDA_1.ME_CODIGO LEFT OUTER JOIN
	                      dbo.MEDIDA MEDIDA_2 ON dbo.PEDIMPDET.ME_GENERICO = MEDIDA_2.ME_CODIGO
	WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') and dbo.PEDIMP.PI_MOVIMIENTO='E' AND (dbo.CONFIGURATIPO.CFT_TIPO IN ('E', 'L', 'M', 'P', 'R', 'S', 'T','O')) AND 
		dbo.VINVENTARIOKARSUM.FE_FECHA>=@fecha and dbo.PEDIMP.PI_FEC_ENT <= @fecha
	GROUP BY  CASE WHEN dbo.PEDIMP.PI_TIPO IN ('C', 'A') THEN isnull(dbo.AGENCIAPATENTE.AGT_PATENTE, '') 
                      + '-' + dbo.PEDIMP.PI_FOLIO ELSE dbo.PEDIMP.PI_FOLIO END, MEDIDA_2.ME_CORTO, MEDIDA_1.ME_CORTO, dbo.ARANCEL.AR_FRACCION, 
	                      dbo.PEDIMPDET.PID_POR_DEF, dbo.PEDIMPDET.PID_DEF_TIP, dbo.SECTOR.SE_CLAVE, dbo.SPI.SPI_CLAVE, dbo.PEDIMPDET.MA_CODIGO, 
	                      dbo.PEDIMPDET.TI_CODIGO, dbo.PEDIMPDET.PID_NOPARTE, dbo.PEDIMP.PI_FEC_ENT, dbo.PEDIMP.PI_FEC_PAG, 
