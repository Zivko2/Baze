...
CREATE PROCEDURE [dbo].[SP_ACTUALIZATASABAJAPCKLIST]  (@PL_codigo1 int, @user1 int=1)   as

SET NOCOUNT ON 
declare @CF_ACTTASACERTO char(1), @CF_ACTTASAPERPPS char(1), @CF_ACTTASAPER8VA char(1),
@user varchar(25), @PL_codigo varchar(50), @pl_fecha varchar(11)
...
select @user=convert(varchar(25),@user1)
	...
	begin
		EXEC SP_ACTUALIZAIMPEXCELPROVEE @PL_codigo1, 'P'
	end
	else
	begin
		...
		set @PL_codigo= convert(varchar(50),@PL_codigo1)
		
		
		select @pl_fecha=convert(varchar(11),pl_fecha,101) from pcklist where pl_codigo=@pl_codigo
		...
		exec ('SELECT    PCKLISTDET.PLD_INDICED, ''AR_IMPMX''=CASE WHEN PCKLISTDET.AR_IMPMX in (select ar_codigo from arancel where pa_codigo=154 and
		ar_fraccion like ''9802%'') then MAESTRO.AR_IMPMX else PCKLISTDET.AR_IMPMX end, 
		PCKLISTDET.PA_CODIGO as PA_ORIGEN, MAESTRO.MA_SEC_IMP,
		convert(varchar(10),PL_FECHA, 101) as PL_FECHA, PCKLISTDET.MA_CODIGO,
		PCKLISTDET.TI_CODIGO, convert(decimal(38,6),-1.0) AS MA_POR_DEFNVO, ''G'' AS MA_DEF_TIPNVO, ISNULL((SELECT SPI_CODIGO FROM PAIS WHERE PA_CODIGO=PCKLISTDET.PA_CODIGO),0) AS SPI_CODIGOPAIS,
			999999 AS AR_FRACCION, convert(decimal(38,6),-1.0) AS PAR_BEN, convert(decimal(38,6),-1.0) AS SA_PORCENT, convert(decimal(38,6),-1.0) AS AR_PORCENT_8VA, PCKLIST.PR_CODIGO
		INTO dbo.['+@user+'TempTasa]
		FROM         PCKLISTDET LEFT OUTER JOIN
		                      PCKLIST ON PCKLISTDET.PL_CODIGO = PCKLIST.PL_CODIGO LEFT OUTER JOIN
		...
		FROM  ['+@user+'TempTasa]  INNER JOIN
			...
			BEGIN
				...
				begin
					...
					WHERE MA_CODIGO IN 
						...
						WHERE     (CERTORIGMP.CMP_IFECHA <= ['+@user+'TempTasa].PL_FECHA) AND (CERTORIGMP.CMP_VFECHA >= ['+@user+'TempTasa].PL_FECHA)
				...
				begin
					...
					WHERE     (['+@user+'TempTasa].TI_CODIGO IN
						...
						FROM         PERMISO INNER JOIN
					                      ...
					                      ['+@user+'TempTasa] TT1 ON MAESTROCATEG.MA_CODIGO = TT1.MA_CODIGO ON 
					                      PERMISODET.MA_GENERICO = MAESTROCATEG.CPE_CODIGO AND PERMISODET.SE_CODIGO = TT1.MA_SEC_IMP INNER JOIN
					                      IDENTIFICA ON PERMISO.IDE_CODIGO = IDENTIFICA.IDE_CODIGO INNER JOIN
					                      ['+@user+'TempTasaSector2] TS ON TT1.AR_IMPMX = TS.AR_CODIGO
						WHERE     (IDENTIFICA.IDE_CLAVE = ''PS'') AND (PERMISODET.PED_REGISTROTIPO = 1) AND (PERMISODET.PED_ID_SUBORD = 0) AND 
						                      (TT1.TI_CODIGO IN
						                          (SELECT     ti_codigo
						                            FROM          configuratipo
						                            WHERE      cft_tipo <> ''P''))
						GROUP BY TT1.MA_CODIGO)')*/
			...
			begin
				...
				begin
					...
					WHERE   (ARANCEL.AR_PORCENT_8VA <> - 1)
						...
						WHERE     IDENTIFICA.IDE_CLAVE = ''C1'' AND (PERMISO.PE_SALDO>0 OR PERMISO.PE_SALDOCOSTOT>0) AND PERMISO.PE_FECHAVENC>='''+@pl_fecha+''')')
				end
				else
				begin
					exec('UPDATE ['+@user+'TempTasa]
			...
			begin
				...
				exec('SELECT     S3.AR_CODIGO, MAX(round(S3.SA_PORCENT,6)) AS SA_PORCENT, (SELECT MIN(S2.SE_CODIGO) FROM TempTasaSector S2 where S2.AR_CODIGO=S3.AR_CODIGO
			...
			exec('UPDATE PCKLISTDET
			SET     PCKLISTDET.SPI_CODIGO= isnull(['+@user+'TempTasa].SPI_CODIGOPAIS,0), 
				PCKLISTDET.PLD_SEC_IMP=0,
				PCKLISTDET.PLD_DEF_TIP= ['+@user+'TempTasa].MA_DEF_TIPNVO
			FROM         ['+@user+'TempTasa] INNER JOIN
			                      PCKLISTDET ON ['+@user+'TempTasa].PLD_INDICED = PCKLISTDET.PLD_INDICED
			WHERE   ['+@user+'TempTasa].MA_DEF_TIPNVO=''P'' AND PCKLISTDET.PL_CODIGO ='+@PL_codigo)
		
		
			exec('UPDATE PCKLISTDET
			SET     PCKLISTDET.SPI_CODIGO= 0, 
				PCKLISTDET.PLD_SEC_IMP= isnull(['+@user+'TempTasa].MA_SEC_IMP,0),
				PCKLISTDET.PLD_DEF_TIP= ['+@user+'TempTasa].MA_DEF_TIPNVO
			FROM         ['+@user+'TempTasa] INNER JOIN
			                      PCKLISTDET ON ['+@user+'TempTasa].PLD_INDICED = PCKLISTDET.PLD_INDICED
			WHERE   ['+@user+'TempTasa].MA_DEF_TIPNVO=''S'' AND PCKLISTDET.PL_CODIGO ='+@PL_codigo)
			...
			FROM         ['+@user+'TempTasa] INNER JOIN
			                      PCKLISTDET ON ['+@user+'TempTasa].PLD_INDICED = PCKLISTDET.PLD_INDICED
			WHERE   ['+@user+'TempTasa].MA_DEF_TIPNVO<>''P'' AND ['+@user+'TempTasa].MA_DEF_TIPNVO<>''S'' AND PCKLISTDET.PL_CODIGO ='+@PL_codigo)
		 ...
		 exec('exec sp_droptable '''+@user+'TempTasaSector''
			...
			exec sp_droptable '''+@user+'TempTasa''')
					...
					/*
					UPDATE PCKLISTDET
					SET PLD_POR_DEF=dbo.GetAdvalorem(PCKLISTDET.AR_IMPMX, PCKLISTDET.PA_CODIGO, PCKLISTDET.PLD_DEF_TIP, PCKLISTDET.PLD_SEC_IMP, PCKLISTDET.SPI_CODIGO)
					WHERE PCKLISTDET.PL_CODIGO =  @PL_codigo
					*/
					UPDATE PCKLISTDET
     					SET PLD_POR_DEF=(CASE WHEN PCKLISTDET.AR_IMPMX in (select ar_codigo from arancel where pa_codigo=154 and ar_fraccion like '9802%') then
								  dbo.GetAdvalorem(MAESTRO.AR_IMPMX,PCKLISTDET.PA_CODIGO, PCKLISTDET.PLD_DEF_TIP, PCKLISTDET.PLD_SEC_IMP, PCKLISTDET.SPI_CODIGO)
                                                         else
								    dbo.GetAdvalorem(PCKLISTDET.AR_IMPMX, PCKLISTDET.PA_CODIGO, PCKLISTDET.PLD_DEF_TIP, PCKLISTDET.PLD_SEC_IMP, PCKLISTDET.SPI_CODIGO)
						         end) , 
				    		    ar_impmx = case when PCKLISTDET.PLD_DEF_TIP = 'R' and 
							             isnull((select ar_impmxR8 from maestro inner join arancel on maestro.ar_impmxR8 = arancel.ar_codigo  where maestro.ma_codigo = PCKLISTDET.ma_codigo and arancel.ar_fraccion like '9802%' ),-100) <>-100 then   
								   (select ar_impmxR8 from maestro where maestro.ma_codigo = PCKLISTDET.ma_codigo)
			  			               else 
						 	            PCKLISTDET.ar_impmx 	
							       end,
						    eq_impmx = case when PCKLISTDET.PLD_DEF_TIP = 'R' and 
							             isnull((select eq_impmxR8 from maestro inner join arancel on maestro.ar_impmxR8 = arancel.ar_codigo  where maestro.ma_codigo = PCKLISTDET.ma_codigo and arancel.ar_fraccion like '9802%' ),-100) <>-100 then   
								  (select eq_impmxR8 from maestro where maestro.ma_codigo = PCKLISTDET.ma_codigo)
							       else 
							 	  PCKLISTDET.eq_impmx 	
								end,
						    me_arimpmx = case when PCKLISTDET.PLD_DEF_TIP = 'R' and 
							             isnull(
										(select arancel.me_codigo from maestro inner join arancel on maestro.ar_impmxR8 = arancel.ar_codigo  where maestro.ma_codigo = PCKLISTDET.ma_codigo and arancel.ar_fraccion like '9802%' )
									    ,-100) <>-100 then   
								  (select arancel.me_codigo from maestro inner join arancel on maestro.ar_impmxR8 = arancel.ar_codigo  where maestro.ma_codigo = PCKLISTDET.ma_codigo and arancel.ar_fraccion like '9802%' )
							       else 
							 	  PCKLISTDET.me_arimpmx 	
			                			end
		     	                from   PCKLISTDET         
					LEFT OUTER JOIN MAESTRO ON PCKLISTDET.MA_CODIGO = MAESTRO.MA_CODIGO 
					WHERE PCKLISTDET.PL_CODIGO =  @PL_codigo
