...
CREATE PROCEDURE [dbo].[SP_ACTUALIZATASABAJAFACTIMP]  (@fi_codigo1 int, @user1 int=1, @ActualizaPR char(1)='N')   as
...
if (select CF_USAPROVEECERTORIG from configuracion)='S'  -- verificando proveedor
	...
	WHERE     (dbo.CERTORIGMPDET.PR_CODIGO IS NOT NULL) AND  (dbo.FACTIMPDET.FI_CODIGO = @fi_codigo1)
	if (select cf_selpaisimp from configuracion)='S'
	begin
		EXEC SP_ACTUALIZAIMPEXCELPROVEE @fi_codigo1, 'F'
	end
	else
	begin
		...
		exec('exec sp_droptable '''+@user+'TempTasa''')
		...
		*/
		...
		WHERE LEFT(ARANCEL.AR_FRACCION, 6)<>''SINFRA'' AND LEFT(ARANCEL.AR_FRACCION, 6)<>''SIN FR''')
			...
			begin
				...
				''UPDATE ['+@user+'TEMPTASA] SET MA_POR_DEFNVO=SA_PORCENT, MA_DEF_TIPNVO=''''S'''' WHERE 
