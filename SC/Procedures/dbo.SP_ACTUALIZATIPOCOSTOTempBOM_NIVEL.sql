...
CREATE PROCEDURE [dbo].[SP_ACTUALIZATIPOCOSTOTempBOM_NIVEL] (@spi_codigo int=22)    as
...
SET NOCOUNT ON 
	...
	begin
		...
		esGravable=case when (bst_trans = 'N') and BOM_STRUCT.bst_hijo in (SELECT CERTORIGMPDET.MA_CODIGO
					         ...
					         WHERE CERTORIGMP.SPI_CODIGO = @spi_codigo and CERTORIGMPDET.PA_CLASE=maestro.pa_origen 
						and LEFT(REPLACE(CERTORIGMPDET.CMP_FRACCION,'.',''),6)=LEFT(REPLACE(ISNULL((SELECT AR_FRACCION FROM ARANCEL WHERE AR_CODIGO=MAESTRO.AR_EXPMX), (SELECT AR_FRACCION FROM ARANCEL WHERE AR_CODIGO=MAESTRO.AR_IMPMX)),'.',''),6)
		...
		esAnadido=case when MAESTRO.MA_REPARA <>'A' then 'N' else 'S' end,
			...
			FROM         BOM_STRUCT INNER JOIN
			                      ...
			                      TempBOM_NIVEL ON BOM_STRUCT.BSU_SUBENSAMBLE = TempBOM_NIVEL.BST_HIJO
	...
	update TempBomGravableNivel	set bst_tipocosto='N'
