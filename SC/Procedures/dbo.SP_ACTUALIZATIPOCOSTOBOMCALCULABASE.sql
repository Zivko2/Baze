...
GO
CREATE PROCEDURE [dbo].[SP_ACTUALIZATIPOCOSTOBOMCALCULABASE] (@spi_codigo int, @bst_pt int)    as
...
SET NOCOUNT ON 
	...
	begin
		SELECT  TempBOM_CALCULABASE.BST_CODIGO, 
		...
		esAnadido=case when MAESTRO.MA_REPARA <>'A' then 'N' else 'S' end,
			esMP=case when TempBOM_CALCULABASE.TI_CODIGO in ('R', 'L', 'M', 'O') or (TempBOM_CALCULABASE.TI_CODIGO ='S' and TempBOM_CALCULABASE.MA_TIP_ENS='C') then 'S' else 'N' end,
			esSUB=case when TempBOM_CALCULABASE.TI_CODIGO ='S' and TempBOM_CALCULABASE.MA_TIP_ENS<>'C' then 'S' else 'N' end, 'Z' as bst_tipocosto
			into dbo.TempBomGravableRep
			FROM         TempBOM_CALCULABASE INNER JOIN
			                      MAESTRO ON TempBOM_CALCULABASE.BST_HIJO = MAESTRO.MA_CODIGO 
			WHERE TempBOM_CALCULABASE.BST_PT=@bst_pt
	...
	begin
		...
		(case when TempBOM_CALCULABASE.pa_codigo in (SELECT pa_codigo FROM TempPaisTLC) then
	...
	from TempBomGravableRep inner join TempBOM_CALCULABASE on TempBomGravableRep.bst_codigo=TempBOM_CALCULABASE.bst_codigo
	where TempBOM_CALCULABASE.bst_tipocosto is null or TempBOM_CALCULABASE.bst_tipocosto<>TempBomGravableRep.bst_tipocosto
