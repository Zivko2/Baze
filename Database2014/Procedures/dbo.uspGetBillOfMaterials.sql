...
FROM [Production].[BillOfMaterials] AS b INNER JOIN [Production].[Product] AS p ON b.[ComponentID] = p.[ProductID]
WHERE b.[ProductAssemblyID] = @StartProduct
AND
...
FROM [BOM_cte] AS cte INNER JOIN [Production].[BillOfMaterials] AS b ON b.[ProductAssemblyID] = cte.[ComponentID]
INNER JOIN [Production].[Product] AS p ON b.[ComponentID] = p.[ProductID]
