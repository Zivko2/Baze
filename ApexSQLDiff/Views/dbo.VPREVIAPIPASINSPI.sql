...
CREATE VIEW dbo.VPREVIAPIPASINSPI
with encryption as
SELECT     TOP 100 PERCENT dbo.AGENCIAPATENTE.AGT_PATENTE collate database_default + '-' + dbo.PEDIMP.PI_FOLIO collate database_default AS PI_PATENTEFOLIO, dbo.PEDIMP.PI_FEC_ENT, 
                      ...
                      dbo.PEDIMPDET.SPI_CODIGO, dbo.PAIS.PA_CORTO, dbo.SPI.SPI_CLAVE AS Expr1
FROM         dbo.ARANCEL RIGHT OUTER JOIN
                      ...
                      dbo.PAIS ON dbo.PEDIMPDET.PA_ORIGEN = dbo.PAIS.PA_CODIGO ON dbo.ARANCEL.AR_CODIGO = dbo.PEDIMPDET.AR_IMPMX RIGHT OUTER JOIN
...
WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') AND (dbo.PEDIMP.PI_MOVIMIENTO = 'E') AND (dbo.CLAVEPED.CP_DESCARGABLE = 'S') AND 
                      (dbo.PEDIMPDET.PID_NOPARTE IS NOT NULL) AND (dbo.PEDIMPDET.PID_DEF_TIP = 'P') AND (dbo.PAIS.SPI_CODIGO = 0 OR
                      dbo.PAIS.SPI_CODIGO IS NULL) AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IA') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IM') AND 
