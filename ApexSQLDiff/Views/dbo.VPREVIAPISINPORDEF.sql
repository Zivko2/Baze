...
CREATE VIEW dbo.VPREVIAPISINPORDEF
with encryption as
SELECT     TOP 100 PERCENT dbo.AGENCIAPATENTE.AGT_PATENTE collate database_default + '-' + dbo.PEDIMP.PI_FOLIO collate database_default AS PI_PATENTEFOLIO, dbo.PEDIMP.PI_FEC_ENT, 
                      ...
                      dbo.VPID_DEF_TIP.CB_LOOKUP AS PID_DEF_TIPT, dbo.PEDIMPDET.PID_POR_DEF
FROM         dbo.ARANCEL RIGHT OUTER JOIN
                      ...
                      dbo.VPID_DEF_TIP ON dbo.PEDIMPDET.PID_DEF_TIP = dbo.VPID_DEF_TIP.CB_KEYFIELD ON 
...
WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') AND (dbo.PEDIMP.PI_MOVIMIENTO = 'E') AND (dbo.CLAVEPED.CP_DESCARGABLE = 'S') AND 
                      (dbo.PEDIMPDET.PID_NOPARTE IS NOT NULL) AND (dbo.PEDIMPDET.PID_POR_DEF = - 1 OR
                      dbo.PEDIMPDET.PID_POR_DEF IS NULL) AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IA') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IM') AND 
