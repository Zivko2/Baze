...
-- reparacion (retrabajo)
CREATE VIEW dbo.VEXHIBIT_A_11Det
with encryption as
SELECT     SUM(CSA_98020040) AS CSA_98020040, CS_CODIGO, FE_CODIGO, AR_FRACCION
FROM         (SELECT     SUM(dbo.FACTEXPDET.FED_NG_USA * dbo.FACTEXPDET.FED_CANT) AS CSA_98020040, 
		...
		WHERE     (dbo.CONFIGURATEMBARQUE.CFQ_TIPO <> 'D') AND (dbo.FACTEXP.TN_CODIGO = 2 OR
		                      ...
		                      dbo.FACTEXP.TN_CODIGO = 5 OR dbo.FACTEXP.TN_CODIGO = 8 OR dbo.FACTEXP.TN_CODIGO = 9) AND (REPLACE(dbo.ARANCEL.AR_FRACCION, '.', '') LIKE '98020040%' OR 
		                      REPLACE(ARANCEL_1.AR_FRACCION, '.', '') LIKE '98020040%')
		GROUP BY dbo.VRELCSFACTEXP.CS_CODIGO, dbo.FACTEXP.FE_CODIGO, ARANCEL_1.AR_FRACCION) EXHIBIT_A_11a
