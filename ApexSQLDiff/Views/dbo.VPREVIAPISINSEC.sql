...
CREATE VIEW dbo.VPREVIAPISINSEC
with encryption as
...
WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') AND (dbo.PEDIMP.PI_MOVIMIENTO = 'E') AND (dbo.CLAVEPED.CP_DESCARGABLE = 'S') AND 
                      (dbo.PEDIMPDET.PID_NOPARTE IS NOT NULL) AND (dbo.PEDIMPDET.PID_DEF_TIP = 'S') AND (dbo.PEDIMPDET.PID_SEC_IMP = 0 OR
                      dbo.PEDIMPDET.PID_SEC_IMP IS NULL) AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IA') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IM') AND 
