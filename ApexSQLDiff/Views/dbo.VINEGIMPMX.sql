...
CREATE VIEW dbo.VINEGIMPMX
with encryption as
SELECT     SUM(dbo.PEDIMPDET.PID_COS_UNIGEN * dbo.KARDESPED.KAP_CANTDESC * dbo.PEDIMP.PI_TIP_CAM) AS COSTOTOTALMPMX, 
                      dbo.INEGI.IG_CODIGO
...
WHERE      (dbo.CONFIGURATIPO.CFT_TIPO IN ('R', 'L', 'M', 'O', 'T')) AND (dbo.CONFIGURACLAVEPED.CCP_TIPO IN ('OC'))
