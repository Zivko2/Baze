...
CREATE VIEW dbo.VPREVIADESINARIMPMX
with encryption as
SELECT     TOP 100 PERCENT dbo.FACTIMP.FI_FOLIO, dbo.FACTIMP.FI_FECHA, dbo.TFACTURA.TF_NOMBRE, dbo.FACTIMPDET.MA_CODIGO, 
                      dbo.FACTIMPDET.FID_NOPARTE, dbo.FACTIMPDET.FID_NOMBRE, dbo.FACTIMPDET.AR_IMPMX, dbo.FACTIMPDET.FID_CANT_ST, 
...
WHERE     (dbo.FACTIMPDET.FID_NOPARTE IS NOT NULL) AND (dbo.FACTIMP.FI_CANCELADO = 'N') AND (dbo.FACTIMPDET.AR_IMPMX = 0 OR
                      dbo.FACTIMPDET.AR_IMPMX IS NULL) AND (dbo.CONFIGURATFACT.CFF_TIPO <> 'ID' AND 
