...
CREATE VIEW dbo.VDESCARGAFAM
with encryption as
SELECT     dbo.FACTEXPDET.FE_CODIGO, dbo.BOM_DESCTEMP.FED_INDICED, dbo.BOM_DESCTEMP.FED_CANT, dbo.MAESTRO.MA_FAMILIAMP, 
...
FROM         dbo.MAESTRO LEFT OUTER JOIN
                      dbo.VMAESTROCOST ON dbo.MAESTRO.MA_FAMILIAMP = dbo.VMAESTROCOST.MA_CODIGO RIGHT OUTER JOIN
...
GROUP BY dbo.FACTEXPDET.FE_CODIGO, dbo.BOM_DESCTEMP.FED_INDICED, dbo.BOM_DESCTEMP.FACTCONV, dbo.BOM_DESCTEMP.FED_CANT, 
                      dbo.BOM_DESCTEMP.BST_TIPODESC, dbo.BOM_DESCTEMP.BST_TIPOCOSTO, dbo.MAESTRO.MA_FAMILIAMP, dbo.MAESTRO.SE_CODIGO, 
                      dbo.MAESTRO.MA_DEF_TIP
HAVING      (SUM(dbo.BOM_DESCTEMP.BST_INCORPOR) > 0) AND (dbo.BOM_DESCTEMP.FED_CANT > 0) AND (dbo.MAESTRO.MA_FAMILIAMP IS NOT NULL)
AND (dbo.MAESTRO.MA_FAMILIAMP >0)
