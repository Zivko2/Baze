...
CREATE VIEW dbo.VPREVIAPISINARIMPMX
with encryption as
...
WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') AND (dbo.PEDIMP.PI_MOVIMIENTO = 'E') AND (dbo.CLAVEPED.CP_DESCARGABLE = 'S') AND 
                      (dbo.PEDIMPDET.PID_NOPARTE IS NOT NULL) AND (dbo.PEDIMPDET.AR_IMPMX = 0 OR
                      dbo.PEDIMPDET.AR_IMPMX IS NULL) AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IA') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IM') AND 
                      (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IB') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IE') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'OC') AND (NOT (dbo.CONFIGURATIPO.CFT_TIPO IN ('Q', 
