...
CREATE VIEW dbo.VPEDIMPCONTRIBUCION
with encryption as
SELECT     PI_CODIGO, CON_CODIGO, ROUND(SUM(PIT_CONTRIBTOTMN), 0) AS PIT_CONTRIBTOTMN, PG_CODIGO, TTA_CODIGO
...
WHERE     (PIT_CONTRIBPOR <> 0) or (CON_CODIGO in(select con_codigo from contribucion where con_clave='15'))
GROUP BY PI_CODIGO, CON_CODIGO, PG_CODIGO, TTA_CODIGO
