...
CREATE VIEW dbo.VPREVIADESINSPI
with encryption as
...
WHERE     (dbo.FACTIMPDET.FID_NOPARTE IS NOT NULL) AND (dbo.FACTIMP.FI_CANCELADO = 'N') AND (dbo.FACTIMPDET.SPI_CODIGO = 0 OR
                      dbo.FACTIMPDET.SPI_CODIGO IS NULL) AND (dbo.FACTIMPDET.FID_DEF_TIP = 'P') AND (dbo.CONFIGURATFACT.CFF_TIPO <> 'ID' AND 
