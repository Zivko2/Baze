...
CREATE VIEW dbo.VPREVIAPISINCOSTO
with encryption as
...
WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') AND (dbo.PEDIMP.PI_MOVIMIENTO = 'E') AND (dbo.PEDIMPDET.PID_COS_UNI = 0 OR
                      dbo.PEDIMPDET.PID_COS_UNI IS NULL) AND (dbo.CLAVEPED.CP_DESCARGABLE = 'S') AND (dbo.PEDIMPDET.PID_NOPARTE IS NOT NULL) AND 
                      (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IA') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IM') AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IB') 
                      AND (dbo.CONFIGURACLAVEPED.CCP_TIPO <> 'IE') AND (NOT (dbo.CONFIGURATIPO.CFT_TIPO IN ('Q', 'H'))) AND (dbo.PEDIMPDET.PID_IMPRIMIR = 'S')
