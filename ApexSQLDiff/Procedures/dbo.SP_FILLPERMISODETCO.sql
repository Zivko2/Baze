...
CREATE PROCEDURE [dbo].[SP_FILLPERMISODETCO] @pecodigo INT    as
...
DELETE FROM PERMISODET WHERE PE_CODIGO=@pecodigo AND PED_REGISTROTIPO=2


DECLARE CUR_PERMISO CURSOR FOR

SELECT 2 AS PED_REGISTROTIPO, CPE_CODIGO, CPE_CORTO, CPE_DESC, 
...
WHERE (CONFIGURATIPO.CFT_TIPO = 'C') AND (CATEGPERMISO.IDE_CODIGO IS NULL OR  CATEGPERMISO.IDE_CODIGO IN 
...
SELECT @CONSECUTIVO=ISNULL(MAX(PED_CONSECUTIVO),0) FROM PERMISODET -- WHERE PE_CODIGO=@pecodigo AND PED_REGISTROTIPO=2
