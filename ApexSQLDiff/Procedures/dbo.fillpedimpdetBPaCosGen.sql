...
CREATE PROCEDURE dbo.[fillpedimpdetBPaCosGen] (@picodigo int, @pi_movimiento char(1), @user int)   as
		...
		begin
			...
			begin
				insert into TempPedimpDetB ( PI_CODIGO, MA_GENERICO, AR_IMPMX, ME_ARIMPMX, ME_GENERICO, PIB_CAN_GEN, PIB_CAN_AR, 
					...
					PIB_CANT, PIB_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, PIB_NOMBRE, PIB_COS_UNIGEN, PIB_PAGACONTRIB,
					PIB_SEC_IMP, PIB_DEF_TIP, SPI_CODIGO, PIB_CTOT_MN, PIB_VAL_RET, PIB_GENERA_EMPDET, PIB_SERVICIO, PIB_CODIGOFACT)
				...
				SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
						...
						round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
				                      PA_ORIGEN, UPPER(MAX(PID_NOMBRE)), PID_COS_UNIGEN, PID_PAGACONTRIB, PID_SEC_IMP, PID_DEF_TIP, SPI_CODIGO, SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET,
				...
				GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
				                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB, PID_SEC_IMP, PID_DEF_TIP, SPI_CODIGO,
						...
						'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
							case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end,
							 EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
				                      PA_ORIGEN, UPPER(PID_NOMBRE), PID_COS_UNIGEN, PID_PAGACONTRIB, PID_SEC_IMP, PID_DEF_TIP, SPI_CODIGO, SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO, PID_CODIGOFACT
				FROM         VFillPedImpDetB
				WHERE     (PI_CODIGO = @picodigo)
				GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
				                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB, PID_SEC_IMP, PID_DEF_TIP, SPI_CODIGO, UPPER(PID_NOMBRE),
					        PA_SAAIM3, IDENTIFICADOR, ME_CLA_PED, PIB_GENERA_EMPDET, PID_SERVICIO, PID_CODIGOFACT
			...
			begin
				insert into TempPedimpDetB ( PI_CODIGO, MA_GENERICO, AR_IMPMX, ME_ARIMPMX, ME_GENERICO, PIB_CAN_GEN, PIB_CAN_AR, 
					PA_PROCEDE, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, PIB_VAL_ADU, PIB_VAL_US, 
					 PIB_VAL_FAC,  AR_EXPFO, PIB_RATEEXPFO, PIB_COS_UNIGRA, PIB_COS_UNIVA, EQ_EXPFO,
					PIB_CANT, PIB_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, PIB_NOMBRE, PIB_COS_UNIGEN, PIB_PAGACONTRIB,
					PIB_SEC_IMP, PIB_DEF_TIP, SPI_CODIGO, PIB_CTOT_MN, PIB_VAL_RET, PIB_GENERA_EMPDET, PIB_SERVICIO)
			...
			begin
				...
				SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
						...
						'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
							case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, 
						EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 				                      
						PA_ORIGEN, UPPER(PID_NOMBRE), PID_COS_UNIGEN, PID_PAGACONTRIB, PID_SEC_IMP, PID_DEF_TIP, SPI_CODIGO, 
						SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO
				FROM         VFillPedImpDetB
				WHERE     (PI_CODIGO = @picodigo)
				GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
				                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB, PID_SEC_IMP, PID_DEF_TIP, SPI_CODIGO, UPPER(PID_NOMBRE),
				...
				begin
					...
					SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
							...
							'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
								case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, 
								EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
					                      PA_ORIGEN, UPPER(MAX(PID_NOMBRE)), PID_COS_UNIGEN, PID_PAGACONTRIB, max(PID_SEC_IMP), max(PID_DEF_TIP), 
						max(SPI_CODIGO), SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO,
						PID_CODIGOFACT
					FROM         VFillPedImpDetB
					WHERE     (PI_CODIGO = @picodigo)
					GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
					                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB,
						        PA_SAAIM3, IDENTIFICADOR, ME_CLA_PED, PIB_GENERA_EMPDET, PID_SERVICIO, PID_CODIGOFACT
					ORDER BY PID_CODIGOFACT, AR_FRACCION, UPPER(MAX(PID_NOMBRE)), PA_SAAIM3, IDENTIFICADOR, ME_CLA_PED, AR_IMPMX, MA_GENERICO
					...
					SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
							...
							'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
								case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
					                      PA_ORIGEN, UPPER(PID_NOMBRE), PID_COS_UNIGEN, PID_PAGACONTRIB, max(PID_SEC_IMP), max(PID_DEF_TIP), 
						max(SPI_CODIGO), SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO,
						PID_CODIGOFACT
					FROM         VFillPedImpDetB
					WHERE     (PI_CODIGO = @picodigo)
					GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
					                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB, UPPER(PID_NOMBRE),
						        PA_SAAIM3, IDENTIFICADOR, ME_CLA_PED, PIB_GENERA_EMPDET, PID_SERVICIO, PID_CODIGOFACT
					ORDER BY PID_CODIGOFACT, AR_FRACCION, UPPER(PID_NOMBRE), PA_SAAIM3, IDENTIFICADOR, ME_CLA_PED, AR_IMPMX, MA_GENERICO
			...
			begin
				...
				begin
					...
					SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
							...
							'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
							case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
					                      PA_ORIGEN, UPPER(MAX(PID_NOMBRE)), PID_COS_UNIGEN, PID_PAGACONTRIB, max(PID_SEC_IMP), max(PID_DEF_TIP), max(SPI_CODIGO), SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO
					FROM         VFillPedImpDetB
					WHERE     (PI_CODIGO = @picodigo)
					GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
					                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB,
				...
				begin
					...
					SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
							...
							'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
							case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
					                      PA_ORIGEN, UPPER(PID_NOMBRE), PID_COS_UNIGEN, PID_PAGACONTRIB, max(PID_SEC_IMP), 
							max(PID_DEF_TIP), max(SPI_CODIGO), SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO
					FROM         VFillPedImpDetB
					WHERE     (PI_CODIGO = @picodigo)
					GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, 
					                      EQ_EXPFO, PID_POR_DEF, PIB_DESTNAFTA, PA_ORIGEN, AR_FRACCION, PID_COS_UNIGEN, PID_PAGACONTRIB, UPPER(PID_NOMBRE),
			...
			begin
				...
				begin
					...
					SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
							...
							'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
							case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
					                      PA_ORIGEN, UPPER(MAX(PID_NOMBRE)), PID_COS_UNIGEN, PID_PAGACONTRIB, max(PID_SEC_IMP), 
							max(PID_DEF_TIP), max(SPI_CODIGO), SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO,
						PID_CODIGOFACT
					FROM         VFillPedImpDetB
					WHERE     (PI_CODIGO = @picodigo)
				...
				begin
					...
					SELECT     @picodigo, MA_GENERICO, AR_IMPMX, MAX(ME_ARIMPMX), MAX(ME_GENERICO), round(SUM(PID_CAN_GEN),3), 
							...
							'a'=case when SUM(PID_CANT)>0 then sum(PIB_COS_UNIGRA*PID_CANT)/SUM(PID_CANT) else 0 end, 
								case when SUM(PID_CANT)>0 then round(sum(PID_COS_UNIVA*PID_CANT)/sum(PID_CANT),6) else 0 end, EQ_EXPFO, round(sum(PID_CANT),3), PID_POR_DEF, PIB_DESTNAFTA, 
					                      PA_ORIGEN, UPPER(PID_NOMBRE), PID_COS_UNIGEN, PID_PAGACONTRIB, max(PID_SEC_IMP), max(PID_DEF_TIP), max(SPI_CODIGO), SUM(PID_CTOT_MN), SUM(PID_VAL_RET), PIB_GENERA_EMPDET, PID_SERVICIO,
						PID_CODIGOFACT
					FROM         VFillPedImpDetB
					WHERE     (PI_CODIGO = @picodigo)
					GROUP BY MA_GENERICO, AR_IMPMX, ES_ORIGEN, ES_DESTINO, ES_COMPRADOR, ES_VENDEDOR, AR_EXPFO, PID_RATEEXPFO, 
