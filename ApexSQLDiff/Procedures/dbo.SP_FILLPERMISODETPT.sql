...
CREATE PROCEDURE [dbo].[SP_FILLPERMISODETPT] @pecodigo INT    as
...
DECLARE CUR_PERMISO CURSOR FOR
	SELECT 1 AS PED_REGISTROTIPO, CPE_CODIGO, CPE_CORTO, CPE_DESC, 
	...
	WHERE (CONFIGURATIPO.CFT_TIPO ='P') AND (CATEGPERMISO.IDE_CODIGO IS NULL OR  CATEGPERMISO.IDE_CODIGO IN 
...
BEGIN
	...
	SELECT @CONSECUTIVO=ISNULL(MAX(PED_CONSECUTIVO),0) FROM PERMISODET -- WHERE PE_CODIGO=@pecodigo AND PED_REGISTROTIPO=1 AND PED_ID_SUBORD=0
