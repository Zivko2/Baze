...
/* sustituye el numero de parte hijo dejando igual la incorporacion, merma y desperdicio */
CREATE PROCEDURE [dbo].[SP_BOMSUSTITUYEigual] (@pt int,@bst_hijo int, @BST_NUEVO INT, @PerCambios CHAR(1))   as
...
@Incorpor decimal(38,6), @Desp decimal(38,6), @Merma decimal(38,6), @bst_tip_ens char(1), @pa_codigo int, @countbom int, @BM_PERFIN datetime, @ma_peso_kg decimal(38,6)

SET @FECHAACTUAL = convert(datetime, convert(varchar(11), getdate(),101));

select @countbom=count(*) from bom_struct where  bsu_subensamble=@pt
...
select @Incorpor=bst_incorpor from bom_struct where bst_hijo=@bst_hijo
and bsu_subensamble= @pt and bst_perini<= @FECHAACTUAL and bst_perfin>= @FECHAACTUAL
...
IF @PerCambios ='S'
	...
	BEGIN
		...
		/* Comentado ya que deberia validar solo si esta vencido MAnuel G. 17-Sep-2010		
