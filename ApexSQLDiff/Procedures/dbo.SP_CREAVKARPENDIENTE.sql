...
CREATE PROCEDURE [dbo].[SP_CREAVKARPENDIENTE] (@FECODIGO INT)   as

DECLARE @fe_codigo varchar(50)

	select @fe_codigo=convert(varchar(50), @FECODIGO)

exec('exec sp_droptable ''VKARPENDIENTE'+@fe_codigo+''', ''V''')



exec('CREATE VIEW dbo.VKARPENDIENTE'+@fe_codigo+' 
as	
...
FROM         KARDESPED
WHERE KAP_FACTRANS=0'+@fe_codigo+' AND
