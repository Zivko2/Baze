...
CREATE PROCEDURE [dbo].[SP_ACTUALIZAESTATUSORDTRABAJO] (@OT_CODIGO INT)    as

SET NOCOUNT ON 
DECLARE @saldo decimal(38,6), @cant decimal(38,6)

	SELECT   @saldo = round(SUM(dbo.ORDTRABAJODET.OTD_SALDO),2), @cant = round(SUM(dbo.ORDTRABAJODET.OTD_SIZELOTE),2)
...
/* actualizamos Estatus de la Orden de Trabajo */
	...
	begin
		...
		begin
			...
			begin
				update ORDTRABAJO
					set ot_estatus = 'N'		-- Nueva Orden
					where OT_CODIGO = @OT_CODIGO
			end
			else
			begin
					update ORDTRABAJO
					set ot_estatus = 'P'			-- Parcialmente cumplida
					where OT_CODIGO = @OT_CODIGO
