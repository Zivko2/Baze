...
CREATE PROCEDURE [dbo].[SP_fillpedexpdetDesp] (@picodigo int, @ccp_tipo varchar(5), @user int)   as
...
/*===================================*/
	...
	*/
	...
	begin
		...
		begin
			...
			begin
				...
				SELECT     @picodigo, VFillPedExpDetDesp.MA_CODIGO, FED_NOPARTE, FED_NOMBRE, FED_NAME, FED_CANT, 
					         'PID_CTOT_DLS'=case when factexp.mo_codigo IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO <> 154 AND MONEDA.PA_CODIGO <> 233) then (FED_COS_TOT*FACTEXP.FE_TIPOCAMBIO)/@PI_TIP_CAM else FED_COS_TOT end,
				                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, 
				                      1, 1,  ISNULL(VFillPedExpDetDesp.AR_EXPMX, 0), ISNULL(VFillPedExpDetDesp.AR_IMPFO, 0), 
				                      FED_RATEIMPFO, 0, FED_DEF_TIP, 
					         0, VFillPedExpDetDesp.TI_CODIGO,  case when @ccp_tipo='CN' AND (select PI_DESP_EQUIPO from pedimp where pi_codigo=@picodigo)='S' then VFillPedExpDetDesp.PA_CODIGO ELSE
						isnull(VPID_DESTNAFTA.PA_CODIGO,233) END, VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), 
				                      isnull(VFillPedExpDetDesp.ME_GENERICO, VFillPedExpDetDesp.ME_CODIGO), 
							...
							round((FED_GRA_EMP + FED_GRA_ADD + FED_GRA_MP + (FED_NG_MP + FED_NG_ADD - (isnull(FED_NG_USA,0)+isnull(FED_NG_MX,0))))*FED_CANT/FED_CANT,6) end) end,
				                      ...
				                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, isnull(FED_NOPARTEAUX,''), VFillPedExpDetDesp.FED_INDICED, CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(VFillPedExpDetDesp.FED_ORD_COMP,'') else '' end,
						'PID_CTOT_MN'=case when factexp.mo_codigo IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 154) then isnull(VFillPedExpDetDesp.FED_COS_TOT,0) else (case when @PI_USA_TIP_CAMFACT<>'S' and factexp.mo_codigo IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(FED_COS_TOT*@PI_TIP_CAM,6) else round(FED_COS_TOT*FACTEXP.FE_TIPOCAMBIO,6) end) end,
						 round(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_GEN,1),6),
						'PID_CAN_AR'=case when isnull(VFillPedExpDetDesp.FED_CANT,0)=0 and VFillPedExpDetDesp.ME_AREXPMX in (select ME_KILOGRAMOS from configuracion) then isnull(VFillPedExpDetDesp.FED_PES_NET,0) 
									else round(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_EXPMX,1),14) end,
						FED_GENERA_EMPDET, VFillPedExpDetDesp.PID_PES_UNIKG, isnull(VFillPedExpDetDesp.FED_PIDSECUENCIA,0), isnull(VFillPedExpDetDesp.MA_SERVICIO,'N')
				FROM         DIR_CLIENTE RIGHT OUTER JOIN
				                      ...
				                      VFillPedExpDetDesp ON VPID_DESTNAFTA.FED_INDICED = VFillPedExpDetDesp.FED_INDICED ON 
				                      FACTEXP.FE_CODIGO = VFillPedExpDetDesp.FE_CODIGO LEFT OUTER JOIN
				                      MAESTRO ON VFillPedExpDetDesp.MA_CODIGO = MAESTRO.MA_CODIGO
				WHERE     (FACTEXP.PI_CODIGO = @picodigo or (FACTEXP.PI_CODIGO = @picodigo and
					VFillPedExpDetDesp.FED_INDICED IN
				...
				AND (VFillPedExpDetDesp.MA_CODIGO IS NOT NULL)
				ORDER BY FACTEXP.FE_CODIGO, VFillPedExpDetDesp.FED_INDICED
			end
			else
			begin
				...
				SELECT     @picodigo, VFillPedExpDetDesp.MA_CODIGO, FED_NOPARTE, FED_NOMBRE, FED_NAME, 
						      FED_CANT, 
					         'PID_CTOT_DLS'=case when factexp.mo_codigo IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO <> 154 AND MONEDA.PA_CODIGO <> 233) then (FED_COS_TOT*FACTEXP.FE_TIPOCAMBIO)/@PI_TIP_CAM else FED_COS_TOT end,
				                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, 
				                      1, 1,  ISNULL(VFillPedExpDetDesp.AR_EXPMX, 0), ISNULL(VFillPedExpDetDesp.AR_IMPFO, 0), 
						...
						'PID_CTOT_MN'=case when factexp.mo_codigo IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 154) then isnull(VFillPedExpDetDesp.FED_COS_TOT,0) else (case when @PI_USA_TIP_CAMFACT<>'S' and factexp.mo_codigo IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(FED_COS_TOT*@PI_TIP_CAM,6) else round(FED_COS_TOT*FACTEXP.FE_TIPOCAMBIO,6) end) end,
						 round(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_GEN,1),6),
	 					'PID_CAN_AR'=case when isnull(VFillPedExpDetDesp.FED_CANT,0)=0 and VFillPedExpDetDesp.ME_AREXPMX in (select ME_KILOGRAMOS from configuracion) then isnull(VFillPedExpDetDesp.FED_PES_NET,0) 
						else round(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_EXPMX,1),14) end, FED_GENERA_EMPDET, VFillPedExpDetDesp.PID_PES_UNIKG, isnull(VFillPedExpDetDesp.MA_SERVICIO,'N')
				FROM         DIR_CLIENTE RIGHT OUTER JOIN
				                      FACTEXP ON DIR_CLIENTE.DI_INDICE = FACTEXP.DI_DESTINI LEFT OUTER JOIN
				                      VPID_DESTNAFTA RIGHT OUTER JOIN
				                      VFillPedExpDetDesp ON VPID_DESTNAFTA.FED_INDICED = VFillPedExpDetDesp.FED_INDICED ON 
		...
		begin
			...
			begin
				...
				begin
					...
					SELECT     @picodigo, VFillPedExpDetDesp.MA_CODIGO, FED_NOPARTE, MAX(FED_NOMBRE), MAX(FED_NAME), 
	  					      SUM(FED_CANT), 
						         'PID_CTOT_DLS'=case when max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO <> 154 AND MONEDA.PA_CODIGO <> 233) then (SUM(FED_COS_TOT)*max(FACTEXP.FE_TIPOCAMBIO))/@PI_TIP_CAM else SUM(FED_COS_TOT) end,
					                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, 
					                      1, 1,  ISNULL(max(VFillPedExpDetDesp.AR_EXPMX), 0), ISNULL(max(VFillPedExpDetDesp.AR_IMPFO), 0), 
					                      FED_RATEIMPFO, 0, FED_DEF_TIP, 
						         0, max(VFillPedExpDetDesp.TI_CODIGO), case when @ccp_tipo='CN' AND (select PI_DESP_EQUIPO from pedimp where pi_codigo=@picodigo)='S' then VFillPedExpDetDesp.PA_CODIGO ELSE
							isnull(VPID_DESTNAFTA.PA_CODIGO,233) END, VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), 
					                      isnull(max(VFillPedExpDetDesp.ME_GENERICO), VFillPedExpDetDesp.ME_CODIGO), 
							...
							round(sum((FED_GRA_EMP + FED_GRA_ADD + FED_GRA_MP + (FED_NG_MP + FED_NG_ADD - (isnull(FED_NG_USA,0)+isnull(FED_NG_MX,0))))*FED_CANT)/sum(FED_CANT),6) end)end,
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, max(isnull(FED_NOPARTEAUX,'')), VFillPedExpDetDesp.FE_CODIGO, CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end,
							'PID_CTOT_MN'=case when max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 154) then SUM(isnull(VFillPedExpDetDesp.FED_COS_TOT,0)) else (case when @PI_USA_TIP_CAMFACT<>'S' and max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(SUM(FED_COS_TOT)*@PI_TIP_CAM,6) else round(SUM(FED_COS_TOT)*max(FACTEXP.FE_TIPOCAMBIO),6) end) end,
							 round(SUM(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_GEN,1)),6),						  
							'PID_CAN_AR'=case when SUM(isnull(VFillPedExpDetDesp.FED_CANT,0))=0 and MAX(VFillPedExpDetDesp.ME_AREXPMX) in (select ME_KILOGRAMOS from configuracion) then round(SUM(isnull(VFillPedExpDetDesp.FED_PES_NET,0)),14) 
								else round(SUM(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_EXPMX,1)),14) end,
							MAX(FED_GENERA_EMPDET),max(VFillPedExpDetDesp.PID_PES_UNIKG), isnull(VFillPedExpDetDesp.MA_SERVICIO,'N')
					FROM         DIR_CLIENTE RIGHT OUTER JOIN
					                      FACTEXP ON DIR_CLIENTE.DI_INDICE = FACTEXP.DI_DESTINI LEFT OUTER JOIN
					                      VPID_DESTNAFTA RIGHT OUTER JOIN
					                      VFillPedExpDetDesp ON VPID_DESTNAFTA.FED_INDICED = VFillPedExpDetDesp.FED_INDICED ON 
					...
					WHERE     (FACTEXP.PI_CODIGO = @picodigo or (FACTEXP.PI_CODIGO = @picodigo and
						...
						WHERE     PEDIMPDET.PID_REGIONFIN <> 'M' GROUP BY FACTEXPDET.FED_INDICED)))
					AND (VFillPedExpDetDesp.MA_CODIGO IS NOT NULL)
					GROUP BY VPID_DESTNAFTA.PA_CODIGO, VFillPedExpDetDesp.MA_CODIGO, FED_NOPARTE,
					                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, VFillPedExpDetDesp.PA_CODIGO,
					                      FED_RATEIMPFO, FED_DEF_TIP/*, VFillPedExpDetDesp.PID_PES_UNIKG*/,
					                      VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end, 
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, VFillPedExpDetDesp.FE_CODIGO, VFillPedExpDetDesp.Ar_fraccion, isnull(VFillPedExpDetDesp.MA_SERVICIO,'N')
					ORDER BY VFillPedExpDetDesp.Ar_fraccion, FED_NOPARTE, MAX(FED_NOMBRE)
				end
				else
				begin
					...
					SELECT     @picodigo, VFillPedExpDetDesp.MA_CODIGO, VFillPedExpDetDesp.FED_NOPARTE, MAX(VFillPedExpDetDesp.FED_NOMBRE), MAX(VFillPedExpDetDesp.FED_NAME), 
	  					      SUM(FED_CANT), 
						         'PID_CTOT_DLS'=case when max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO <> 154 AND MONEDA.PA_CODIGO <> 233) then (SUM(FED_COS_TOT)*max(FACTEXP.FE_TIPOCAMBIO))/@PI_TIP_CAM else SUM(FED_COS_TOT) end,
					                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, 
					                      1, 1, ISNULL(max(VFillPedExpDetDesp.AR_EXPMX), 0), ISNULL(max(VFillPedExpDetDesp.AR_IMPFO), 0), 
					                      VFillPedExpDetDesp.FED_RATEIMPFO, 0, FED_DEF_TIP, 
							0, max(VFillPedExpDetDesp.TI_CODIGO),case when @ccp_tipo='CN' AND (select PI_DESP_EQUIPO from pedimp where pi_codigo=@picodigo)='S' then VFillPedExpDetDesp.PA_CODIGO ELSE
							isnull(VPID_DESTNAFTA.PA_CODIGO,233) END, VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), 
					                      isnull(max(VFillPedExpDetDesp.ME_GENERICO), VFillPedExpDetDesp.ME_CODIGO), 
						        'PID_COS_UNIVA'=case when @ccp_tipo='IR' or sum(FED_CANT)=0 then 0 else (case when @PI_USA_TIP_CAMFACT<>'S' and max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(sum(PID_COS_UNIVAUSD*FED_CANT)*@PI_TIP_CAM/sum(FED_CANT),6) else round(sum(PID_COS_UNIVAUSD*FED_CANT)*max(FACTEXP.FE_TIPOCAMBIO)/sum(FED_CANT),6)  end) end, 
							...
							round((sum((FED_GRA_EMP + FED_GRA_ADD + FED_GRA_MP + (FED_NG_MP + FED_NG_ADD - (isnull(FED_NG_USA,0)+isnull(FED_NG_MX,0))))*FED_CANT)/sum(FED_CANT))/@PI_TIP_CAM,6) else 
							round(sum((FED_GRA_EMP + FED_GRA_ADD + FED_GRA_MP + (FED_NG_MP + FED_NG_ADD - (isnull(FED_NG_USA,0)+isnull(FED_NG_MX,0))))*FED_CANT)/sum(FED_CANT),6) end) end,
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, max(isnull(VFillPedExpDetDesp.FED_NOPARTEAUX, '')), VFillPedExpDetDesp.FE_CODIGO, CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end,
							'PID_CTOT_MN'=case when max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 154) then SUM(isnull(VFillPedExpDetDesp.FED_COS_TOT,0)) else (case when @PI_USA_TIP_CAMFACT<>'S' and max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(SUM(FED_COS_TOT)*@PI_TIP_CAM,6) else round(SUM(FED_COS_TOT)*max(FACTEXP.FE_TIPOCAMBIO),6) end) end,
							 round(SUM(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_GEN,1)),6),
							'PID_CAN_AR'=case when SUM(isnull(VFillPedExpDetDesp.FED_CANT,0))=0 and MAX(VFillPedExpDetDesp.ME_AREXPMX) in (select ME_KILOGRAMOS from configuracion) then round(SUM(isnull(VFillPedExpDetDesp.FED_PES_NET,0)),14) 
								else round(SUM(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_EXPMX,1)),14) end,
					...
					WHERE     (FACTEXP.PI_RECTIFICA =@picodigo or (FACTEXP.PI_RECTIFICA = @picodigo and
						...
						WHERE     PEDIMPDET.PID_REGIONFIN <> 'M' GROUP BY FACTEXPDET.FED_INDICED)))
						AND (VFillPedExpDetDesp.MA_CODIGO IS NOT NULL)
					GROUP BY VPID_DESTNAFTA.PA_CODIGO, VFillPedExpDetDesp.MA_CODIGO, VFillPedExpDetDesp.FED_NOPARTE, 
					                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, VFillPedExpDetDesp.PA_CODIGO,
					                      VFillPedExpDetDesp.FED_RATEIMPFO/*, VFillPedExpDetDesp.PID_PES_UNIKG*/,
					                      VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), FED_DEF_TIP,
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, VFillPedExpDetDesp.FE_CODIGO, CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end, VFillPedExpDetDesp.Ar_fraccion, isnull(VFillPedExpDetDesp.MA_SERVICIO,'N')
					ORDER BY VFillPedExpDetDesp.Ar_fraccion, FED_NOPARTE, MAX(FED_NOMBRE)
				end
			end
			else
				...
				begin
					...
					SELECT     @picodigo, VFillPedExpDetDesp.MA_CODIGO, VFillPedExpDetDesp.FED_NOPARTE, MAX(VFillPedExpDetDesp.FED_NOMBRE), MAX(VFillPedExpDetDesp.FED_NAME), 
	  					      SUM(FED_CANT), 
						         'PID_CTOT_DLS'=case when max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO <> 154 AND MONEDA.PA_CODIGO <> 233) then (SUM(FED_COS_TOT)*max(FACTEXP.FE_TIPOCAMBIO))/@PI_TIP_CAM else SUM(FED_COS_TOT) end,
							 VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, 
					                      1, 1, ISNULL(max(VFillPedExpDetDesp.AR_EXPMX), 0), ISNULL(max(VFillPedExpDetDesp.AR_IMPFO), 0), VFillPedExpDetDesp.FED_RATEIMPFO, 0, FED_DEF_TIP,
						          0, max(VFillPedExpDetDesp.TI_CODIGO),case when @ccp_tipo='CN' AND (select PI_DESP_EQUIPO from pedimp where pi_codigo=@picodigo)='S' then VFillPedExpDetDesp.PA_CODIGO ELSE
					isnull(VPID_DESTNAFTA.PA_CODIGO,233) END, VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), 
					                      isnull(max(VFillPedExpDetDesp.ME_GENERICO), VFillPedExpDetDesp.ME_CODIGO), 
						        'PID_COS_UNIVA'=case when @ccp_tipo='IR' or sum(FED_CANT)=0 then 0 else (case when @PI_USA_TIP_CAMFACT<>'S' and max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(sum(PID_COS_UNIVAUSDGI*FED_CANT)*@PI_TIP_CAM/sum(FED_CANT),6) else round(sum(PID_COS_UNIVAUSDGI*FED_CANT)*max(FACTEXP.FE_TIPOCAMBIO)/sum(FED_CANT),6)  end) end, 
					...
					GROUP BY VPID_DESTNAFTA.PA_CODIGO, VFillPedExpDetDesp.MA_CODIGO, VFillPedExpDetDesp.FED_NOPARTE, 
					                      VFillPedExpDetDesp.ME_CODIGO, VFillPedExpDetDesp.MA_GENERICO, VFillPedExpDetDesp.PA_CODIGO,
					                      VFillPedExpDetDesp.FED_RATEIMPFO, FED_DEF_TIP/*, VFillPedExpDetDesp.PID_PES_UNIKG*/,
					                      VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233),
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, VFillPedExpDetDesp.FE_CODIGO, CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end, VFillPedExpDetDesp.Ar_fraccion, isnull(VFillPedExpDetDesp.MA_SERVICIO,'N')
					ORDER BY VFillPedExpDetDesp.Ar_fraccion, FED_NOPARTE, MAX(FED_NOMBRE)
				end
			...
			begin
				...
				begin
					...
					isnull(VPID_DESTNAFTA.PA_CODIGO,233) END, VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), 
							...
							round((sum((FED_GRA_EMP + FED_GRA_ADD + FED_GRA_MP + (FED_NG_MP + FED_NG_ADD - (isnull(FED_NG_USA,0)+isnull(FED_NG_MX,0))))*FED_CANT)/sum(FED_CANT))/@PI_TIP_CAM,6) else 
							round(sum((FED_GRA_EMP + FED_GRA_ADD + FED_GRA_MP + (FED_NG_MP + FED_NG_ADD - (isnull(FED_NG_USA,0)+isnull(FED_NG_MX,0))))*FED_CANT)/sum(FED_CANT),6) end) end,
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, max(isnull(VFillPedExpDetDesp.FED_NOPARTEAUX,'')), CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end,
							'PID_CTOT_MN'=case when max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 154) then SUM(isnull(VFillPedExpDetDesp.FED_COS_TOT,0)) else (case when @PI_USA_TIP_CAMFACT<>'S' and max(factexp.mo_codigo) IN (SELECT MO_CODIGO FROM MONEDA WHERE MONEDA.PA_CODIGO = 233) then round(SUM(FED_COS_TOT)*@PI_TIP_CAM,6) else round(SUM(FED_COS_TOT)*max(FACTEXP.FE_TIPOCAMBIO),6) end) end,
							 round(SUM(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_GEN,1)),6),
 							'PID_CAN_AR'=case when SUM(isnull(VFillPedExpDetDesp.FED_CANT,0))=0 and MAX(VFillPedExpDetDesp.ME_AREXPMX) in (select ME_KILOGRAMOS from configuracion) then round(SUM(isnull(VFillPedExpDetDesp.FED_PES_NET,0)),14) 
								else round(SUM(isnull(VFillPedExpDetDesp.FED_CANT,0)*isnull(VFillPedExpDetDesp.EQ_EXPMX,1)),14) end, MAX(FED_GENERA_EMPDET), max(VFillPedExpDetDesp.PID_PES_UNIKG), isnull(VFillPedExpDetDesp.MA_SERVICIO,'N'), factexp.fe_codigo
					FROM         DIR_CLIENTE RIGHT OUTER JOIN
					                      FACTEXP ON DIR_CLIENTE.DI_INDICE = FACTEXP.DI_DESTINI LEFT OUTER JOIN
					                      VPID_DESTNAFTA RIGHT OUTER JOIN
					                      VFillPedExpDetDesp ON VPID_DESTNAFTA.FED_INDICED = VFillPedExpDetDesp.FED_INDICED ON 
					...
					GROUP BY VPID_DESTNAFTA.PA_CODIGO, VFillPedExpDetDesp.MA_CODIGO, VFillPedExpDetDesp.FED_NOPARTE,
					                      ...
					                      VFillPedExpDetDesp.FED_RATEIMPFO, FED_DEF_TIP/*, VFillPedExpDetDesp.PID_PES_UNIKG*/,
					                      VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233),
					                      VFillPedExpDetDesp.SE_CODIGO, VPID_DESTNAFTA.PID_DESTNAFTA, CASE WHEN @PICF_SAAIDETDIVPO='S' then isnull(FED_ORD_COMP,'') else '' end, VFillPedExpDetDesp.Ar_fraccion, isnull(VFillPedExpDetDesp.MA_SERVICIO,'N'), factexp.fe_codigo
					ORDER BY VFillPedExpDetDesp.Ar_fraccion, FED_NOPARTE, MAX(FED_NOMBRE)
				end
				else
				begin
					...
					isnull(VPID_DESTNAFTA.PA_CODIGO,233) END, VFillPedExpDetDesp.SPI_CODIGO, isnull(DIR_CLIENTE.PA_CODIGO,233), 
	 					...
	 					MAX(FED_GENERA_EMPDET),max(VFillPedExpDetDesp.PID_PES_UNIKG), isnull(VFillPedExpDetDesp.MA_SERVICIO,'N'), factexp.fe_codigo
	 				FROM         DIR_CLIENTE RIGHT OUTER JOIN
					                      FACTEXP ON DIR_CLIENTE.DI_INDICE = FACTEXP.DI_DESTINI LEFT OUTER JOIN
					                      VPID_DESTNAFTA RIGHT OUTER JOIN
					                      VFillPedExpDetDesp ON VPID_DESTNAFTA.FED_INDICED = VFillPedExpDetDesp.FED_INDICED ON 
	...
	begin
		...
		where PID_CANT >0 AND ME_ARIMPMX=36
