...
CREATE PROCEDURE dbo.[SP_GENAUTFACTEXPPO] (@PI_CODIGO INT, @FI_CODIGO INT, @Caso INT)   as
...
--Yolanda 2008-12-26
--Esta es la variable para llevar el conteo del folio de las fact. export que inician con 'REM'
declare @contador varchar(25)
declare @fi_tipo char(1)
declare @tf_codigoFE int
...
/*
		...
		WHERE     (FACTIMP.PI_CODIGO = @PI_CODIGO) and FACTIMPDET.FID_ORD_COMP NOT IN (SELECT FED_ORD_COMP FROM FACTEXPDET INNER JOIN FACTEXP ON FACTEXPDET.FE_CODIGO=FACTEXP.FE_CODIGO
...
*/
		...
		FROM         FACTIMPDET INNER JOIN
		                      FACTIMP ON FACTIMPDET.FI_CODIGO = FACTIMP.FI_CODIGO
		--yolanda 2008-12-30
		--en caso de que unifiquen los dos stored procedures que son identicos	
		WHERE   ((@Caso = 1 and FACTIMP.PI_CODIGO = @PI_CODIGO) or    (@Caso <> 1 and FACTIMP.FI_CODIGO = @FI_CODIGO))
			AND FACTIMPDET.FID_ORD_COMP NOT LIKE 'OP%'
			AND FACTIMPDET.FID_ORD_COMP NOT LIKE 'OT%'			
		GROUP BY FACTIMPDET.CL_CODIGO, FACTIMP.FI_FECHA, FACTIMP.TQ_CODIGO, FACTIMPDET.FID_ORD_COMP
	...
	begin
		...
		SET @folio='REM'
			...
			--Yolanda 2008-12-26
			--asigna el folio a las fact. export, la variable @contador actualizara mas tarde el contador de la tabla de CONFIGURAFOLIOENT
			...
			begin
				...
				--Yolanda 2008-12-30
			        --asigna el folio a las fact. export, la variable @contador actualizara mas tarde el contador de la tabla de CONFIGURAFOLIOENT
				set @contador =replicate('0', 5-(len(@Numero)))+convert(varchar(50),@Numero) 
			...
			--Yolanda 2008-12-26
			--Actualiza el contador del folio de las fact. export que inician con 'REM' para que cuando haga una manualmente le asigne el folio correcto.
			update CONFIGURAFOLIOENT
			set cfe_contador = convert(integer, @contador) +1
			from CONFIGURAFOLIOENT
			where tf_codigo = 12 and cfe_tipo = 'V' and cfe_prefijo = 'REM'
			...
			------WHERE  (FACTIMP.PI_CODIGO = @PI_CODIGO)
			--yolanda 2008-12-30
			--en caso de que unifiquen los dos stored procedures que son identicos	
			WHERE  ((@Caso = 1 and FACTIMP.PI_CODIGO = @PI_CODIGO) or    (@Caso <> 1 and FACTIMP.FI_CODIGO = @FI_CODIGO))
			AND FACTIMPDET.FID_ORD_COMP NOT LIKE 'OP%'
