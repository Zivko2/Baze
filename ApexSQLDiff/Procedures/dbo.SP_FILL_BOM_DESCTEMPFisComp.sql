...
CREATE PROCEDURE [dbo].[SP_FILL_BOM_DESCTEMPFisComp] (@FED_INDICED INT, @BST_PT Int, @BST_ENTRAVIGOR DateTime, @FED_CANT decimal(38,6), @CODIGOFACTURA INT, @MENSAJE1 char(1)='N' output)   as
...
begin
    ...
    if @MA_Struct is not null
       and @MA_Struct > 0
       and exists(select [BST_Hijo]
                  from   BOM_Struct
                  where  [BSU_Subensamble] = @MA_Struct
                         and [BST_PerIni] <= @BST_EntraVigor
                         --glr (16-sep-2010)
                         --se estaba usando la fecha inicial, en vez de la final
		  ...
		  BEGIN  
				...
				begin			
					...
					begin
						...
						values(@BST_HIJO, @BST_INCORPOR, 'S', 'S', @FACTCONV, @ME_GEN, 'C', @FED_CANT, @CODIGOFACTURA, 1,
					...
					begin
						...
						begin
							...
							insert into bom_desctemp (BST_HIJO, BST_INCORPOR, BST_DISCH, TI_CODIGO, FACTCONV, ME_GEN, 
							   MA_TIP_ENS, FED_CANT, FE_CODIGO, BST_NIVEL, BST_TIPODESC, BST_PERTENECE, FED_INDICED, BST_PT, BST_ENTRAVIGOR)
							values(@BST_HIJO, @BST_INCORPOR, 'S', 'S', @FACTCONV, @ME_GEN, 'C', @CantAlcanza, @CODIGOFACTURA, 1,
							@BST_TIPODESC, @BST_PT, @FED_INDICED, @BST_PT, @BST_ENTRAVIGOR)
