...
CREATE PROCEDURE [dbo].[SP_CALCULABOMCOSTOMODIF] (@FECHA DATETIME,@GuardaHist char(1)='N', @spi_codigo int =22)   as
...
select @fechaactual=convert(varchar(11),getdate(),101)
	...
	TRUNCATE TABLE TempBOM_NIVEL
		...
		AND BOM_STRUCT.BSU_SUBENSAMBLE NOT IN
			...
			(SELECT MA_CODIGO FROM MAESTRO WHERE convert(varchar(11),MAESTRO.MA_ULTIMAMODIF,101)=@FECHA)
		GROUP BY BOM_STRUCT.BSU_SUBENSAMBLE
