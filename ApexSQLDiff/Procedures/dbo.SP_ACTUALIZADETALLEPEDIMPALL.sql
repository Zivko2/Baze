...
CREATE PROCEDURE [dbo].[SP_ACTUALIZADETALLEPEDIMPALL]   as

SET NOCOUNT ON 
declare @picodigo int, @pimovimiento char(1), @cp_codigo int, @ccp_tipo varchar(5), @pi_tipo char(1), @pi_estatus char(1),
@pi_fec_pag datetime--, @Tabla table

	--exec sp_droptable 'PEDIMPACT'

	--create @Tabla table (PI_CODIGO int, PI_MOVIMIENTO char(1), cp_codigo int, PI_TIPO char(1), PI_ESTATUS char(1), PI_FEC_PAG datetime)
