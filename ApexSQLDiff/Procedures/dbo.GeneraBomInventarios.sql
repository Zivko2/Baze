...
CREATE PROCEDURE [dbo].[GeneraBomInventarios] (@fechaini varchar(10), @fechafin varchar(10))   as
...
dbcc checkident (retrabajotemp, reseed, @CONSECUTIVO) WITH NO_INFOMSGS
...
UPDATE FACTEXPDET
SET FED_RETRABAJO='E'
WHERE FED_RETRABAJO<>'E' and fed_indiced in (select fed_indiced from RetrabajoMod)
...
print 'Insertando en la tabla RETRABAJO'
	...
	SELECT @RE_INDICER+RE_INDICER, TIPO_FACTRANS, FETR_CODIGO, FETR_INDICED, MA_HIJO, RE_NOPARTE, RE_NOMBRE, RE_NAME, RE_INCORPOR,
		TI_HIJO, ME_CODIGO, MA_GENERICO, ME_GEN, FACTCONV, FETR_RETRABAJODES, RE_INCORPORGEN, NFTR_NAFTA, PA_ORIGEN
	FROM RETRABAJOTemp
