...
CREATE PROCEDURE [dbo].[SP_INVENTARIOFECHAMA] (@macodigo int, @cft_tipo char(1), @picodigo int, @fecha datetime)   as
...
declare @NoPedimento varchar(50),  @FechaPedimento datetime, @FechaPagoPedimento datetime,  @PID_saldoCANT decimal(38,6), @ME_CORTO varchar(5), 
	...
	WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') and dbo.PEDIMP.PI_MOVIMIENTO='E' and
		dbo.PEDIMPDET.ma_codigo=@macodigo AND dbo.PEDIMP.PI_FEC_ENT<=@fecha
	...
	UNION
	-- se suma la cantidad que ha sido descargada despues de la fecha en cuestion en base a la fecha de la factura de exportacion (se le debe de sumar)
	SELECT       CASE WHEN dbo.PEDIMP.PI_TIPO IN ('C', 'A') THEN isnull(dbo.AGENCIAPATENTE.AGT_PATENTE, '') + '-' + dbo.PEDIMP.PI_FOLIO ELSE dbo.PEDIMP.PI_FOLIO END AS NoPedimento, dbo.PEDIMP.PI_FEC_ENT AS FechaPedimento, dbo.PEDIMP.PI_FEC_PAG AS FechaPagoPedimento,
	...
	WHERE     (dbo.PEDIMP.PI_ESTATUS <> 'R') and dbo.PEDIMP.PI_MOVIMIENTO='E' and dbo.PEDIMPDET.ma_codigo=@macodigo AND dbo.VINVENTARIOKARSUM.FE_FECHA>=@fecha
