...
CREATE PROCEDURE dbo.[fillpedimpcont] (@picodigo int, @user int)    as
...
FROM         dbo.PEDIMPCONT
	...
	WHERE     (dbo.PEDIMPDET.PI_CODIGO = @picodigo) AND (dbo.FACTIMP.PI_CODIGO = @picodigo) and dbo.PEDIMPDET.PID_INDICED is not null
		and (dbo.FACTIMPCONT.FIC_MARCA is not null or dbo.FACTIMPCONT.FIC_MODELO is not null or dbo.FACTIMPCONT.FIC_SERIE is not null or
